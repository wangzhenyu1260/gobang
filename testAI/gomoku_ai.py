#!/usr/bin/python
# -*- coding: UTF-8 -*-
# https://github.com/ZitongMao/gomoku-ai
import random
...
class gomokuAI(object):
        ...
        for axis in directions:
            for (xdirection, ydirection) in axis:
                ...
                if ydirection != 0 and (i + ydirection < 0 or i + ydirection >= N):
                    break
                if self.__gomoku.get_chessMap()[i + ydirection][j + xdirection] != ChessboardState.EMPTY:
                    return True
                ...
                if self.__gomoku.get_chessMap()[i + ydirection * 2][j + xdirection * 2] != ChessboardState.EMPTY:
                    return True
            ...
            if state == ChessboardState.BLACK:
                for pattern in WHITE_5PATTERNS:
                    if sublist(pattern, temp):
                        return True
            if state == ChessboardState.WHITE:
    ...
    def generate(self):
        ...
        for i in xrange(N):
            for j in xrange(N):
                if self.__gomoku.get_chessMap()[i][j] != ChessboardState.EMPTY:
                    continue  # only search for available spots
                if not self.has_neighbor(self.__gomoku.get_chessMap()[i][j], i, j):
                    continue

                if self.__currentState == ChessboardState.WHITE:
                    nextState = ChessboardState.BLACK
                else:
                    nextState = ChessboardState.WHITE
    ...
    def one_step(self):
        ...
        if not i is None and not j is None:
            if self.__gomoku.get_chessboard_state(i, j) != ChessboardState.EMPTY:
                self.one_step()
            else:
                self.__gomoku.set_chessboard_state(i, j, self.__currentState)
                list2.append((i, j))
